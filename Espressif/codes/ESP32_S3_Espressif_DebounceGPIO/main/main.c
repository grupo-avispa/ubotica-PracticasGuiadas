...
#define LED_PIN (gpio_num_t)CONFIG_LED1 // onboard LED for ESP-WROM-32
#define LED_PIN_2 (gpio_num_t)CONFIG_LED2 // another LED output pin

#define timer_T_ms (unsigned long)CONFIG_DEBOUNCE_TIME // Debouncing timer period in milliseconds
...
void DebounceCallback(TimerHandle_t xTimer) {
    // Re-enable the interrupt for the input pin after the debounce period
...
void LED_Control_Task(void *params)
{
    ...
    while (true)
    {
        if (xQueueReceive(interruptQueue, &pinNumber, portMAX_DELAY))
        {
            ...
            // Disable interrupt to avoid bouncing issues
            ESP_ERROR_CHECK(gpio_set_intr_type(INPUT_PIN, GPIO_INTR_DISABLE)); 
            // Clear the queue to avoid multiple triggers during debounce. 
            // WARNING: This will discard ALL GPIO interrupt events that occurred during the debounce period, not just the one that triggered the timer.
            xQueueReset(interruptQueue); 
            // Start debounce timer immediately
...
void app_main()
{
    ...
    debounceTimer = xTimerCreate(
        ...
        pdFALSE,                                   // NO autoreload (single shot timer)
        NULL,                                      // Timer ID
    ...
    ESP_LOGI(TAG, "Debounce timer created with period: %lu ms", timer_T_ms);
